...
const MaterialDropdownManagerFunctional: React.FC = () => {
  ...
  const handleDelete = async (section: string, value: string) => {
    ...
    switch (section) {
      case 'estados':
        newConfig.estados = newConfig.estados.filter(item => item.value !== value);
        break;
      case 'tiposCuerda':
        newConfig.tiposCuerda = newConfig.tiposCuerda.filter(item => item.value !== value);
        break;      case 'tiposAnclaje':
        newConfig.tiposAnclaje = newConfig.tiposAnclaje.filter(item => item.value !== value);
        // También eliminar subcategorías relacionadas
        delete newConfig.subcategoriasAnclaje[value];
        break;
      case 'categoriasVarios':
        newConfig.categoriasVarios = newConfig.categoriasVarios.filter(item => item.value !== value);
        // También eliminar subcategorías relacionadas
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const newItem: DropdownOption = {
      ...
      switch (activeSection) {
        case 'estados':
          const estadoIndex = newConfig.estados.findIndex(item => item.value === editingItem.value);
          if (estadoIndex !== -1) {
            newConfig.estados[estadoIndex] = newItem;
          }
          break;
        case 'tiposCuerda':
          const cuerdaIndex = newConfig.tiposCuerda.findIndex(item => item.value === editingItem.value);
          if (cuerdaIndex !== -1) {
            newConfig.tiposCuerda[cuerdaIndex] = newItem;
          }
          break;        case 'tiposAnclaje':
          const anclajeIndex = newConfig.tiposAnclaje.findIndex(item => item.value === editingItem.value);
          if (anclajeIndex !== -1) {
          ...
          break;
        case 'categoriasVarios':
          const categoriaIndex = newConfig.categoriasVarios.findIndex(item => item.value === editingItem.value);
          if (categoriaIndex !== -1) {
