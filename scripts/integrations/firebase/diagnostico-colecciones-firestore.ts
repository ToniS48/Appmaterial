/**
 * Script de diagnóstico TypeScript para verificar la detección de colecciones de Firestore
 * Ejecuta pruebas de detección y muestra estadísticas detalladas
 */

import { firestoreDynamicService } from '../src/services/firestore/FirestoreDynamicService';

async function diagnosticoColecciones(): Promise<void> {
  ...
  try {
    ...
    const collectionesToTest = ['material_deportivo', 'usuarios', 'actividades', 'prestamos'] as const;
    
    for (const collection of collectionesToTest) {
      try {
        const documents = await firestoreDynamicService.getCollectionDocuments(collection as any, 1);
...
if (require.main === module) {
  diagnosticoColecciones().catch(console.error);
}

export { diagnosticoColecciones };
